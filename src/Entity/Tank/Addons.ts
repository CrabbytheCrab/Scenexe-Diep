...
import TankDefinitions, { addonId, BarrelDefinition } from "../../Const/TankDefinitions";
...
class QuadrupletAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        const base = new AutoTurret(owner, undefined);
        base.baseSize = 50/1.5
        for(let barrel of base.turret)barrel.delete()
        for (const barrelDefinition of TankDefinitions[Tank.Quadro].barrels) {
            const def = Object.assign({}, barrelDefinition, {recoil: 0, width:  barrelDefinition.width/1.5, size:  barrelDefinition.size/1.5, offset: barrelDefinition.offset/1.5});
            def.bullet = Object.assign({}, def.bullet, { damage: 0.25 });
            base.turret.push(new Barrel(base, def));
        }
    }
}
/** SPIESK addon. */
