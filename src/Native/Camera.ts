...
export class CameraEntity extends Entity {
    ...
    public setLevel(level: number) {
        ...
        this.sizeFactor = Math.pow(1.0075, level - 1);
    ...
    public tick(tick: number) {
        if (Entity.exists(this.cameraData.values.player)) {
            ...
            if (this.cameraData.values.player instanceof TankBody) {
                ...
                this.cameraData.movementSpeed = player.definition.speed * 2.55 * Math.pow(1.05, this.cameraData.values.statLevels.values[Stat.MovementSpeed]) / Math.pow(1.012, this.cameraData.values.level - 1)
            }
        } else {
